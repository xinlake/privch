...
bool isPort(String value) {
  int port;

  try {
    port = int.parse(value, radix: 10);
  } catch (e) {
    return false;
  }

  if (!RegExp(r'^[0-9]+$').hasMatch(value) || port <= 0 || port > 65535) {
    return false;
  }
...
bool isURL(String str,
  ...
  var protocol, user, auth, host, hostname, portStr, path, query, hash, split;
...
bool isPostalCode(String text, String locale, bool orElse()) {
  ...
  return pattern != null ? pattern.hasMatch(text) : orElse();
}
